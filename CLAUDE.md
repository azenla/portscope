...
PortScope is a macOS-only SwiftUI app (target `macOS 26.5`, Swift 5, default actor isolation = `MainActor`) that introspects the host's hardware buses via IOKit. The original focus was Thunderbolt — controllers, routers (switches), ports, adapters, downstream PCIe/USB devices, hop tables / tunnels, and bandwidth allocations — and it has since expanded to a full USB explorer (host controllers, hubs, devices, interfaces with bandwidth / speed / class info), a unified **Physical Ports** view that reports each USB-C port's live operating mode, **per-receptacle accessory state** pulled from `IOAccessoryManager` (active transports CC / USB 2 / USB 3 / CIO / DisplayPort, USB-PD voltage and current, plug orientation, displayport HPD, cable e-marker VID/PID/manufacturer), and now **Displays / Bluetooth / PCIe** sections with grouped SoC coprocessors under Internal Hardware.
...
Data flow is one direction: **IOKit → Scanners → SystemSnapshot → View Model → SwiftUI views**. Seven scanners now run per refresh (Thunderbolt, USB, Accessory, InternalHardware, Bluetooth, Display, PCI). TB and USB produce `TBNode` trees through a single shared builder + formatter so node metadata stays consistent; the accessory scanner is shape-different (one struct per physical receptacle, not a tree) and gets merged into `PhysicalPort` by `TopologyMapper`; the InternalHardware scanner pulls SoC-internal blocks (I²C / SPI buses, the smart battery, and named SoC coprocessors — Secure Enclave, Always-On Processor, ANE, ISP, DCP, NAND controller, SMC, etc.) out of `AppleARMIODevice` and **groups them thematically** for the Internal Hardware sidebar section. The three new scanners feed their own subsystem snapshots into `SystemSnapshot { bluetooth, displays, pcie, … }`.
...
- `Services/BluetoothScanner.swift` — sources the Bluetooth subsystem by spawning `system_profiler -xml SPBluetoothDataType -detailLevel full` (via `Process`) and parsing the plist. The IORegistry's `IOBluetoothHCIController` carries almost no useful properties — chipset name, firmware version, paired-device list, RSSIs, and battery levels all live behind SPBluetoothDataType, which is the same source System Information uses. Produces a `BluetoothSnapshot { controller, connected, paired }`. **Sandbox must stay off** — `Process` won't spawn under app-sandbox.

- `Services/DisplayScanner.swift` — matches `IOMobileFramebufferShim` (Apple Silicon) and `IOFramebuffer` (Intel). Extracts: `DisplayWidth/Height`, refresh range from each `TimingElements[].VerticalAttributes.PreciseSyncRate` (16.16 fixed-point Hz — 7864320 ≈ 120 Hz), preferred ColorMode `Depth` (8/10-bit), `color-accuracy-index`, and HDR support flags. The `IOMFBDisplayRefresh` dict has refresh-step / idle-interval numbers but they're internal pacing knobs, **not the panel's refresh range** — use `TimingElements` instead. `IONameMatched` like `disp0,t603x` distinguishes the built-in (disp0) from external engines (dispext0..N). Idle external engines still surface so the user can see how many displays the silicon can drive.

- `Services/PCIScanner.swift` — walks every `IOPCIDevice` and builds a parent-pointer map by climbing the IOService plane up to 12 hops looking for the nearest enclosing `IOPCIDevice`. Devices with no PCI parent are roots; everything else hangs off its enclosing bridge. Decodes `class-code` (3-byte little-endian: ProgIF, Subclass, Class) into a friendly label via `pciClassLabel`, and unpacks `IOPCIExpressLinkStatus` / `IOPCIExpressLinkCapabilities` (bits 0-3 = speed, bits 4-9 = width). **Saturates to 0xF / 0x3F when there's no link** — `decodeLinkStatus` filters values outside the valid PCIe spec ranges (speeds 1..6, widths 1/2/4/8/12/16/32) so the UI doesn't show "Gen 15 ×63". `pciVendorName` carries a small hand-rolled vendor lookup (Apple, Broadcom, Genesys Logic, Samsung, etc.) — the PCI vendor database is enormous and dragging it in would be overkill for the IDs that actually show up on Macs.
...
- `Models/USBModels.swift` — `USBSpeed` (with `rateMbps`, `rateLabel`, `accentColor`), `USBDeviceClass`, formatters (`usbSpeedLabel`, `usbBcdVersion`, `usbDeviceClassLabel`), `USBSnapshot` (with `tbContext` map), `PhysicalPortMode`, and the top-level **`SystemSnapshot { tb, usb, accessories, internalHardware, bluetooth, displays, pcie, capturedAt }`** that the view model owns.

- `Models/BluetoothModels.swift` — `BluetoothSnapshot { controller, connected, paired }`, `BluetoothController` (address / chipset / firmware / transport / supported services), `BluetoothDevice` (per-device dict from SPBluetoothDataType, including battery levels for AirPods-style triples — `batteryLevelLeft / Right / Case`), `BluetoothDeviceCategory` (audio / keyboard / pointer / gamepad / phone / tablet / computer / watch / display / other — used for sidebar icons + sort priority). The category is inferred from SP's `device_minorType` string ("Headphones", "Magic Trackpad", "Gamepad", …) because IOKit alone can't tell us this.

- `Models/DisplayModels.swift` — `DisplaySnapshot { displays }`, `DisplayInfo` (one entry per `IOMobileFramebufferShim`; carries `isBuiltIn`, `isConnected`, pixel size, refresh-rate range, bit depth, HDR support, mode count, plus the backing TBNode for the Developer-details disclosure).

- `Models/PCIModels.swift` — `PCISnapshot { roots }`, `PCINode` (one per IOPCIDevice — `kind: rootBridge | bridge | endpoint`, vendor/device/subsystem IDs, decoded `classCode/subclassCode/progIF`, link speed/width, BDF, slot name, recursive children). Formatters `pciLinkSpeedLabel` / `pciLinkSpeedShortLabel` map speed codes 1..6 → "PCIe 1.0 (2.5 GT/s)" / "Gen 1" etc. `pciVendorName` is the in-app vendor lookup; `pciClassLabel` decodes base-class codes to USB / Network / Bridge / Mass Storage / etc.

- `Models/InternalHardwareModels.swift` — `InternalHardwareSnapshot` (battery manager, MagSafe, I²C / SPI buses, **`coprocessorGroups: [SoCCoprocessorGroup]`** plus a flat `socCoprocessors` accessor). `SoCCoprocessorCategory` is the thematic grouping: `.displayAndGraphics` (dcp, dcpext, disp, dispext, gfx-asc, sgx) / `.mediaImage` (isp, ane, jpeg, scaler) / `.mediaVideo` (avd, ave) / `.storageMemory` (ans, mcc) / `.securityPower` (sep, aop, pmgr, pmp, smc, aic) / `.radios` (wlan, bluetooth) / `.other`. `InternalHardwareScanner.categorise(name:)` does the bucketing by device-tree name prefix.
...
- `ViewModels/PortScopeViewModel.swift` — `@MainActor ObservableObject`. Owns the `SystemSnapshot`, runs all scanners off-main via `Task.detached`, debounces hot-plug rescans, and exposes selection plus `node(for:)` / `parent(of:)` lookups. `selectionRoots` walks **TB controllers + USB controllers + TB-tunneled PCIe/USB flat lists + I²C/SPI buses + SoC coprocessors + display nodes + flattened PCI tree + battery**, so every sidebar row resolves to a node.

  **`PhysicalPortSelector`** (also in this file) mints synthetic `TBNodeID`s (high bits `0xC0DE_C0DE_…`) for "Physical Port N" sidebar rows so they don't collide with real registry entry IDs. **`BluetoothSelector`** does the same for the Bluetooth controller (`0xB7E0_…`) and paired devices (`0xB7E1_…` ⊕ FNV-1a hash of the BD_ADDR — deterministic so selection survives rescans, unlike `String.hashValue` which is randomised per launch). `ContentView.detail` checks the synthetic-ID predicates first, then PCINode lookup, then DisplayInfo lookup, then falls through to `vm.node(for: sel)`.
...
- `Views/PropertyTableView.swift` — generic key/value renderer for the Developer-details disclosure. Filterable, expandable rows for `Data`/array/dict values, includes a hex+ASCII dump for `Data`. Uses `TBNode.formatValue(_:_:)` which knows how to pretty-print both TB keys (`Adapter Type`, `Link Bandwidth`, `Current Link Speed`) and USB keys (`bcdUSB`, `bDeviceClass`, `Device Speed`, `idVendor`, `idProduct`), and now `compatible` / `IONameMatch` / `IONameMatched` arrays via `prettyCompatibleString` (joins with " · " so JPEG codec's `("jpeg,t8110jpeg","s5l8920x")` reads as `jpeg,t8110jpeg · s5l8920x`).

- `Views/BluetoothViews.swift` — `BluetoothControllerView` (hero + chipset stat grid + profile chips + connected/paired device lists) and `BluetoothDeviceView` (hero + per-device stats + battery rows with linear `ProgressView` for AirPods-style triples). Battery percent is parsed from SP's `"88%"`-formatted strings — keep the raw string for display and percent only for the bar.

- `Views/DisplayViews.swift` — `DisplayDetailView` (hero + stat grid + Timing Modes card listing each unique resolution/refresh combo from `TimingElements`, with the kernel's preferred mode marked). Also defines **`DeveloperDisclosureCard`** — a reusable wrapper around `PropertyTableView` with the same "Developer details" header `DetailView` uses inline, so Display / PCIe / future detail views can offer the raw-IORegistry drill-down without duplicating layout.

- `Views/PCIViews.swift` — `PCIDeviceView` (hero + stat grid with vendor/class/BDF/slot/link, downstream-device list, dev-details disclosure). Uses `PCINode` directly (not `TBNode`) so it shows the decoded class label / vendor name / link generation rather than raw IOPCI properties.
...
- **`IOPCIDevice.IOName` is shared across every PCI bridge** — every `pci-bridge0`, `pci-bridge1`, `pcic0-bridge`, `pcic1-bridge`, `pcic2-bridge` has `IOName = "pci-bridge"`. Use the device-tree `name` property (a Data blob, NOT a string — has to be `unwrapData(props["name"])`-decoded) for unique identification. `PCIScanner.makeLabels` looks at `name` first, then `IOName`, then `node.title` for fallback labels like "PCIe Host Bridge 0" vs "Thunderbolt Root Port".

- **`IOPCIExpressLinkStatus` saturates to 0xF / 0x3F on bridges with no negotiated link** — host bridges fronting empty Thunderbolt slots, etc. Decoded naively this reads as "Gen 15 ×63", which is nonsense. `PCIScanner.decodeLinkStatus` filters speeds outside 1..6 and widths outside `{1, 2, 4, 8, 12, 16, 32}` and returns nil for the field — so callers can show "—" or hide the link line entirely.

- **`IOMFBDisplayRefresh` is NOT the panel's refresh range.** The keys `displayMinRefreshInterval` / `displayMaxRefreshInterval` are internal pacing knobs for the DCP (the MacBook Pro XDR panel reports ~5.6 to ~28 Hz here, which is wrong — the panel does 24..120 Hz). The actual modes live in `TimingElements[*].VerticalAttributes.PreciseSyncRate` as 16.16 fixed-point Hz (`7864320 / 65536 = 120 Hz`). `DisplayScanner` walks every TimingElement to compute the real min/max.

- **`Bluetooth` data has to come from `system_profiler SPBluetoothDataType -xml`, not IOKit.** `IOBluetoothHCIController`'s IORegistry property dict carries almost nothing useful — no chipset name, no firmware version, no paired-device list, no RSSI, no battery levels. `BluetoothScanner` spawns `system_profiler` via `Process` and parses the plist; this requires app-sandbox off (which it already is). On future macOS releases watch for this path breaking — the SP plist schema is mostly stable but Apple has reshuffled keys in the past (`device_minorType`, `device_batteryLevelLeft`, etc.). Currently mapped under `wrapper["DeviceName"]["device_*"]` per device.

- **The `compatible` IORegistry property is an array of device-tree match strings**, e.g. `("jpeg,t8110jpeg", "s5l8920x")`. `IORegValue.display` for arrays renders the whole tuple syntax; `prettyCompatibleString` (in `Models/TBModels.swift`) joins them with " · " so the JPEG codec reads as `jpeg,t8110jpeg · s5l8920x` instead of `("jpeg,t8110jpeg","s5l8920x")`. Some kernel paths serialise the same value as a NUL-separated `Data` blob — `prettyCompatibleString` handles both.

- **`AAPL,slot-name` is a NUL-terminated Data blob, not a CFString.** `props["AAPL,slot-name"]?.asString` returns nil — you have to fall through to `unwrapData(props["AAPL,slot-name"])` to get "Slot- 0..2" for the Thunderbolt root ports. Same trap for `name` on PCI bridges.

- **`SoCCoprocessorGroup` categories are by device-tree name prefix, not class name.** `InternalHardwareScanner.matches(name:prefix:)` accepts an exact match or a prefix followed by digits only — so `disp0` matches `disp` but `disp` doesn't grab `dispext0` (that's `dispext`). New silicon may add prefixes; add them to the bucket lists in `categorise(name:)` rather than rewriting the matcher. Anything unknown ends up in `.other` rather than being dropped so we don't go silent on future generations.
