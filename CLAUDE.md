...
Views in `Views/`: `SidebarView` is seven sections (Physical Ports, Thunderbolt, USB, Displays, Bluetooth, PCIe, Internal Hardware). The three system-internal sections (Displays, Bluetooth, Internal Hardware) are gated on `@AppStorage("showAllDevices")` — the **Settings → Show All Devices** toggle, mirrored by CLI `--all`. `DetailView` dispatches on `TBNodeKind` and hosts the **Developer details** disclosure (raw IORegistry dump via `PropertyTableView`). `PhysicalPortDetailView` is the unified per-port view used when selection is a `PhysicalPortSelector` synthetic ID.
