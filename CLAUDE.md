...
**USB-C Topology** (`Views/USBCTopologyView.swift`): a Mac-centered radial fan-out. The Mac (`MacHub`, marketing name from `MacPortCatalog.current`) sits in the middle; host routers are balanced left/right (`RadialTopology.balancedSplit`, greedy heaviest-to-lighter side) into two columns of `PortPanel`s. A `Canvas` behind the nodes draws a curved Bézier cable from the Mac edge to each panel's inner edge — coloured/weighted by the negotiated link gen (0x2 TB5 purple, 0x4 TB4 blue, 0x8 TB3 teal, idle = thin dashed grey), active links get a gradient + glow. Connector geometry comes from `anchorPreference(.bounds)` collected into `ConnectorAnchorsKey` and resolved in a `backgroundPreferenceValue` `GeometryReader` — keep the connector layer *inside* the scaled content so it zooms with the nodes. Two header toggles (`@AppStorage` `topoShowTBInternals`, `topoShowIntermediateHubs`, both default off): the first reveals the per-router adapter chips + tunnels, the second expands the cascaded USB hub chain (`flattenUSBNodes`). **`AdapterGrid`'s adaptive `LazyVGrid` needs a bounded width** — call sites wrap it in `.frame(width: 320)` because the panels are `.fixedSize()` (unbounded width would collapse the grid to one column). Everything is clickable (`DTTSelection`: mac / hostRouter / deviceRouter / adapter / tunnel / usbDevice / display / pcie); the always-visible right **inspector** shows a host overview (counts + tap-to-jump attached list) when nothing/the Mac is selected, and per-item detail otherwise — a clicked USB device gets a curated summary plus the full `PropertyTableView` behind `RawPropertyDisclosure`. USB rows still flatten up front to `USBTreeRow`s drawn in a single VStack (no recursive Layout — that hung on deep dock chains); `DeviceBlockView` recurses for daisy chains (a struct, not a `some View` function). The model is built eagerly in `init` (no empty first frame) and rebuilt via `.task(id: capturedAt)`. Pinch zoom via `MagnificationGesture` + auto-fit via `fitScaleFor(content:canvas:)`. **Always pass measured sizes by value into auto-fit** — `@State` writes aren't visible inside the same closure that wrote them.
